...
class AchievementsFragment : Fragment() {
    ...
    override fun onResume() {
        ...
        bind(binding.firstBadge, results.isNotEmpty(), getString(R.string.achievement_first))
        bind(binding.tenBadge, results.size >= 10, getString(R.string.achievement_ten))
        bind(binding.highBadge, results.any { it.score >= 95 }, getString(R.string.achievement_high))
        bind(binding.streakBadge, streak >= 3, getString(R.string.achievement_streak))
    }

    private fun bind(view: TextView, unlocked: Boolean, label: String) {
        view.text = if (unlocked) "🏅  $label" else "🔒  $label"
